  ...
  Token.findOne({ token: req.params.userToken }, (err, token) => {
    ...
    User.findOne({ _id: token._userId }, (err, user) => {
      ...
      if (user.verified === true)
        return res
          .status(400)
          .render("login", { error: "This user has already been verified." });
...
exports.user_tokenResend = (req, res) => {
  ...
  if (errors.length > 0) res.status(400).render("resend", { errors });
  else {
    User.findOne({ email: email }, (err, user) => {
      ...
      if (user.verified)
        return res
          .status(400)
          .render("resend", { error_msg: "This account is already verified." });
...
exports.user_forgotPwd = (req, res) => {
  ...
  if (errors.length > 0) res.status(400).render("forgotPwd", { errors });
  else {
    User.findOne({ email: email }, (err, user) => {
      if (err) {
        return res.status(500).send({ msg: err.message });
...
exports.user_changePwd = (req, res) => {
  ...
  if (errors.length > 0) {
    return res
      .status(400)
      .render("changePwd", { errors, token: req.params.userToken });
  } else {
    Token.findOne({ token: req.params.userToken }, (err, token) => {
      if (err) {
        return res.status(500).send({ msg: err.message });
